		...
		<nav class="   px-2.5 pt-1.5 backdrop-blur-xl drag-region select-none">
			<div class=" flex items-center gap-1">
				...
				<div class="flex w-full">
					<div
						class="workspace-nav flex gap-1 scrollbar-none overflow-x-auto w-fit text-center text-sm font-medium rounded-full bg-transparent py-1 touch-auto pointer-events-auto"
					>
						{#if $user?.role === 'admin' || $user?.permissions?.workspace?.models}
							<a
								draggable="false"
								aria-current={$page.url.pathname.includes('/workspace/models') ? 'page' : null}
								class="min-w-fit p-1.5 whitespace-nowrap {$page.url.pathname.includes('/workspace/models')
									? ''
						...
						{#if $user?.role === 'admin' || $user?.permissions?.workspace?.knowledge}
							<a
								...
								class="min-w-fit p-1.5 whitespace-nowrap {$page.url.pathname.includes('/workspace/knowledge')
									? ''
									: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition select-none"
						...
						{#if $user?.role === 'admin' || $user?.permissions?.workspace?.prompts}
							<a
								...
								class="min-w-fit p-1.5 whitespace-nowrap {$page.url.pathname.includes('/workspace/prompts')
									? ''
									: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition select-none"
						...
						{#if $user?.role === 'admin' || $user?.permissions?.workspace?.skills}
							<a
								...
								class="min-w-fit p-1.5 whitespace-nowrap {$page.url.pathname.includes('/workspace/skills')
									? ''
									: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition select-none"
						...
						{#if $user?.role === 'admin' || $user?.permissions?.workspace?.tools}
							<a
								...
								class="min-w-fit p-1.5 whitespace-nowrap {$page.url.pathname.includes('/workspace/tools')
									? ''
									: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition select-none"
