<script>
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_gabomagpt_version ?? '0.0.0', WEBUI_VERSION)) {
			console.log('Version is lower than required');
			toast.error(
				$i18n.t(
					'GabomaGPT version (v{{GABOMAGPT_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
						GABOMAGPT_VERSION: WEBUI_VERSION,
						REQUIRED_VERSION: manifest?.required_gabomagpt_version ?? '0.0.0'
					}
				)
			);
			return;
	...
	onMount(() => {
		window.addEventListener('message', async (event) => {
			if (
				!['https://gabomagpt.com', 'https://www.gabomagpt.com', 'http://localhost:9999'].includes(
					event.origin
				)
			)
				return;
