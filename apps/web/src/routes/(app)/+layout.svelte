<script lang="ts">
	...
	onMount(async () => {
		...
		// GabomaGPT — 2e visite : afficher l'UpgradeModal
		gabomaStore.incrementVisitCount();
		const state = $gabomaStore;
		if (state.visitCount === 2 && state.plan === 'flash') {
			setTimeout(() => gabomaStore.openUpgradeModal(), 1500);
		}
		// Jetons épuisés : pousser vers upgrade
		if (state.tokens <= 0 && state.plan === 'flash') {
			setTimeout(() => gabomaStore.openUpgradeModal(), 800);
		}
	});
...
<!-- GabomaGPT — Modal paiement global -->
{#if showPaymentModal}
	<PaymentModal />
{/if}

<!-- GabomaGPT — Modal upgrade plans -->
{#if showUpgradeModal}
	<UpgradeModal />
{/if}
