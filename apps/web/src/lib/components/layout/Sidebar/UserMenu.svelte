	...
	<slot name="content">
		...
		>
			...
			>
				...
				<div class=" self-center truncate">{$i18n.t('Préférences')}</div>
			</DropdownMenu.Item>
			...
			>
				...
				<div class=" self-center truncate">{$i18n.t('Chroniques Archivées')}</div>
			</DropdownMenu.Item>
			...
			{#if role === 'admin'}
				...
				>
					...
					<div class=" self-center truncate">{$i18n.t('Gouvernance')}</div>
				</DropdownMenu.Item>
			{/if}
			...
			{#if help}
				...
				{#if $user?.role === 'admin'}
					...
					<DropdownMenu.Item
						as="a"
						href="https://github.com/gabomagpt/gabomagpt/releases"
						target="_blank"
						draggable="false"
						class="flex rounded-xl py-1.5 px-3 w-full hover:bg-gray-50 dark:hover:bg-gray-800 transition cursor-pointer select-none"
						id="chat-share-button"
