<script lang="ts">
	...
	import IbogaToggle from '../sidebar/IbogaToggle.svelte';
...
{#if $showSidebar}
	<div
		...
		id="sidebar"
		class="h-screen max-h-[100dvh] min-h-screen select-none glass {$showSidebar
	...
	>
		...
		>
			...
			>
				...
				>
					<img
						...
						src="{WEBUI_BASE_URL}/gabomagpt-logo.jpeg"
						class="sidebar-new-chat-icon size-7 rounded-lg"
						alt="GabomaGPT"
					/>
				</a>

				<a href="/" class="flex flex-1 px-1.5 items-center" on:click={newChatHandler}>
					<div
						id="sidebar-webui-name"
						class="self-center font-display font-semibold tracking-tight text-[15px]"
					>
						<span class="text-[var(--accent)]">Gaboma</span><span class="text-[var(--gabon-blue)]">GPT</span>
					</div>
				</a>
				<!-- GabomaGPT — Arbre Iboga AI comme toggle sidebar -->
				<IbogaToggle
					open={$showSidebar}
					onToggle={() => showSidebar.set(!$showSidebar)}
				/>
			...
			<div
				class="relative flex flex-col flex-1 overflow-y-auto scrollbar-hidden scroll-fade pt-3 pb-3"
				on:scroll={(e) => {
				...
				<div class="pb-1.5">
					...
					<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
						<button
							...
							class="group grow flex items-center space-x-3 rounded-2xl px-2.5 py-2 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 dark:hover:from-blue-950/30 dark:hover:to-purple-950/30 transition outline-none border border-transparent hover:border-blue-200/50 dark:hover:border-blue-800/30"
							on:click={() => {
								showSearch.set(true);
							}}
							draggable="false"
						...
						>
							<div class="self-center relative">
								...
								<span class="absolute -top-1 -right-1.5 size-2 rounded-full bg-gradient-to-br from-blue-400 to-purple-500 ring-2 ring-gray-50 dark:ring-gray-950"></span>
							</div>

							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class="self-center text-sm font-primary bg-gradient-to-r from-gray-800 to-gray-600 dark:from-gray-200 dark:to-gray-400 bg-clip-text">{$i18n.t('Search')}</div>
							</div>
			...
			<div class="px-2 pt-2 pb-2 sticky bottom-0 z-10 -mt-3 sidebar">
			<div
				class="sidebar-bg-gradient-to-t bg-linear-to-t from-gray-50 dark:from-gray-950 to-transparent from-50% pointer-events-none absolute inset-0 -z-10 -mt-6"
			></div>
			<div class="flex flex-col gap-2 font-primary">
				<!-- GabomaGPT — Badge plan + jetons -->
				<PlanBadge />

				<!-- GabomaGPT — Alerte jetons bas -->
				{#if lowTokens}
					<UpgradePrompt />
				{/if}
				...
				{#if $user !== undefined && $user !== null}
					...
					>
						<div
							class="flex items-center rounded-2xl py-2 px-1.5 w-full hover:bg-[rgba(255,255,255,0.04)] transition-colors duration-200"
						>
							...
							<div class="self-center font-medium text-sm text-[var(--text-primary)]">{$user?.name}</div>
						</div>
					</UserMenu>
				{/if}
