...
{#key message.id}
	...
	>
		...
		<div class="flex-auto w-0 pl-1 relative">
			<Name>
				<Tooltip content={getGabomaGPTModelName(model?.id ?? message.model, model?.name)} placement="top-start">
					<span id="response-message-model-name" class="line-clamp-1 text-black dark:text-white">
						{getGabomaGPTModelName(model?.id ?? message.model, model?.name)}
					</span>
				</Tooltip>
					...
					>
						{#if message.done || siblings.length > 1}
							...
							{#if $mobile && message?.done}
								<Tooltip content="Partager WhatsApp" placement="bottom">
									<button
										...
										class="{isLastMessage || ($settings?.highContrastMode ?? false)
											...
											: 'invisible group-hover:visible'} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition"
										on:click={() => {
											const text = message.content?.slice(0, 500) + (message.content?.length > 500 ? '...' : '');
											const url = `https://wa.me/?text=${encodeURIComponent(`GabomaGPT:\n${text}\n\nhttp://31.220.80.217:3000`)}`;
